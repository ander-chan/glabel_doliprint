...
Printer::Printer(std::pair<int, int> ids): context(nullptr), printer(nullptr), ir(Printer::imageResolution::HIGH){
    ...
    for (size_t idx = 0; idx < count; ++idx) {
        ...
        if(desc.idVendor == ids.first && desc.idProduct == ids.second){
            ...
            int config=0;
            int e = libusb_get_configuration(printer,&config);
                if(e!=0)
                {
                    std::cout <<("\n***Error in libusb_get_configuration\n") << std::endl;

                }
                  std::cout <<"\nConfigured value : "<<config<< std::endl;

                if(config != 1)
                {
                    libusb_set_configuration(printer, 1);
                    if(e!=0)
                    {
                        std::cout <<("Error in libusb_set_configuration\n") << std::endl;

                    }
                    else
                        std::cout <<("\nDevice is in configured state!") << std::endl;
                }
            break;
        }
    ...
    int err = libusb_set_configuration(printer, 0);
    if (err) {
         std::cout << "conf error! " <<libusb_error_name(err)<<libusb_strerror(LIBUSB_ERROR_IO)<< std::endl;
    }
    */
}
...
void Printer::raw(std::string text){
    ...
    std::cout << "len:  " << totalChars<<std::endl;
     std::cout << "endpoint:  " <<std::hex<<(1 | LIBUSB_ENDPOINT_IN) <<std::endl;
    int actual = 0;
    //unsigned char buffer[64] = {0x1d,0x6b,0x040,0x00,0x00};
    int r = libusb_bulk_transfer(printer, (1 | LIBUSB_ENDPOINT_IN), (unsigned char *) &text[0], totalChars, &actual, 5000);
    //libusb_interrupt_transfer(printer, (1 | LIBUSB_ENDPOINT_IN), (unsigned char *) &text[0], totalChars, &actual, 0);


    if(r != 0 || actual != totalChars){

        std::cout << "Write Error " << libusb_error_name(r)<<std::endl;
